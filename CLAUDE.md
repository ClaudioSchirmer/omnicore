...
- **Pagination direction (Relay)** — `first`/`after` page forward, `last`/`before` page backward. `last` is the only arg carrying direction without a cursor, so `buildCriteria` sets `ReadCriteria.Backward` for it (`first`/`after`/`before` map 1:1 to `Limit`/`After`/`Before`); the reader's `backward := c.Backward || c.Before != ""` then yields the LAST N from the end for `last:N` (`pageInfo.hasNextPage:false`). REST has no `last` and never sets `Backward` — it stays cursor-driven and unchanged. A forward+backward arg mix (`first`+`last`, `last`+`after`, `after`+`before`) or a non-positive page size is rejected pre-dispatch by `paginationArgConflict` → `SchemaViolationNotification` (semantic Schema), naming the offending arg.
