...
| Topic | Section | Essence |
|---|---|---|
| `TableSchema` — mandatory explicit Go-field↔column map; managed columns; the three-name contract (managed slots surface under FIXED Go names); boot checks; drives write+criteria+scan+view | `table-schema.html` | `NewTableSchema[T]`; no inference; one declaration, every consumer. |
