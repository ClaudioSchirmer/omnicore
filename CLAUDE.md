...
>
> 1. **Every change to this module needs explicit maintainer approval first.** EnumDescription the change + motivation + impact, wait for approval, then edit. No exception for "obvious", "small", "cosmetic", or "a consumer needs it". If a consumer is urgent, work around it in the consumer — never patch the framework "for now".
