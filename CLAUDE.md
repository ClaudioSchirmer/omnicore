...
| Auto query handlers (`QueryWithParams`/`QueryByID`), filter operators, the reserved control keys (`first`/`last`/`after`/`before`/`orderBy`/`fields`/`search`/`includeArchived`/`onlyTotal` — DTO opt-in per endpoint, enforced on EVERY surface by the canonical control gateway `queryschema.ValidateControls`), the Relay envelope (`totalCount`/`hasNextPage`/`hasPreviousPage`/`startCursor`/`endCursor`), computed fields (`computed:"Src"` — derived by `FromQueryResult`, sources pushed down, not sortable/filterable), tabular export (CSV/XLSX) | `auto-query-handlers.html` | Allowlist by tag; one response-projection seat `func(TResult) TResp` shared by every surface; the Response DTO is the single wire authority (it decides the export's columns and the one `?fields=` vocabulary); keyset pagination with the Relay direction pair. |
