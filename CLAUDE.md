...
| Export a view query as a downloadable file (CSV today) | `fwweb.HandleQueryAsCSV(d.Pipeline, requests.FindXxxRequest{}, view.ExportPlan(), d.Translator, view.ResolveMaxExportRows(d.Config.Query.MaxExportRows), "<filename>", handler, export.WithDelimiter(';'))` paired with `fwopenapi.MountRaw(... text/csv ...)`. Reuses the SAME Request DTO + query handler as the JSON list; renders hierarchical (one column per nesting level), headers from `labelKey`, `?fields=` narrows columns; user pagination ignored, full set capped at `query.maxExportRows` / `ViewDefinition.MaxExportRows`. Format is a pluggable `web/export.Encoder` (XLSX later = new encoder). See "Tabular export". |
