...
| Aggregate root + value objects, transparent dispatch, symmetric cascade, child validation, `CollectionName` | `aggregate-persistence.html` | `AggregateRootProvider` opt-in; aggregate is the event unit (granularity B); depth = 1; a child DECLARES its collection name (no pluralization rule anywhere). |
