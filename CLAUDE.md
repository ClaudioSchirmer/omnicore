...
| Topic | Section | Essence |
|---|---|---|
| Rules DSL, `BuildRules`, `EntityMode`/`Modes()`, `actionName`, notifications, parameterized vars, labels, `CompleteAsArchive` | `rules-dsl.html` | Mode-scoped closures; `Get*` family takes `actionName`; sealed `ValidEntity`. An `IfUpdate` rule may finish the write as an archive — domain-only, panics elsewhere, sealed onto the `Updatable` like `IsPartial`; `IfArchive` does not re-fire. |
