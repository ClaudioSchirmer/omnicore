...
| Read joins | `r.WithJoins(read.InnerJoin(schema).On("fk").Field("Go", "col"))` on the repository lets every loader read — and any relational view over it — filter, sort and load across a foreign key into another aggregate; never a write path | [read-joins](https://claudioschirmer.github.io/omnicore/#read-joins) |
