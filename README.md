...
- **Handler invariance — HTTP and Kafka are interchangeable transports.** Write a handler once (`pipeline.Handler[TCmd, TResult]` — Auto Command Handler or hand-rolled). Mount it on an HTTP route via `fwweb.HandleCommandWithBody`, on a cross-service integration receiver via `reg.From(source).On(eventKey, sample, handler)`, or on both simultaneously. The handler stays invariant; the Request DTO with `ToCommand()` adapts the wire payload to the same Cmd; the framework supplies the canonical write-side async path (transactional emission via `fwintegration.Dispatch(...WithTx(tx))` from a `BeforeCommit` closure, at-least-once delivery with per-consumer-group dedup, operator-driven retry surface). Migrate an endpoint between transports without rewriting the handler. See the [documentation site](https://claudioschirmer.github.io/omnicore/#integration-events) → Integration events.
