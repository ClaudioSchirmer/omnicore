...
// NOTE on the Postgres-backed failure registry: the subscriber's
// recordFailure / resolveFailures / RetryPendingFailures reach Postgres through
// the public s.pg.Pool() (concrete *pgxpool.Pool), NOT the injectable pgxPool
// seam — Pool() yields nil for a unit fake. Driving those Exec/Query branches
// would panic on the nil pool, so they are not unit-reachable here; the
// underlying SQL helpers (RecordUpstreamFailure / ResolveUpstreamFailures /
// ListPendingUpstreamFailuresByTopic) are covered directly in
// upstream_failures_test.go via a pgExec fake. These tests therefore give the
// subscriber a nil pg handle (the best-effort writers take their nil-guard
// branch) while still composing through a fake-PG composer for the ripple.

// This file drives infra/upstream_subscriber.go (previously ~0%) entirely
// in-process: a real UpstreamSubscriber over a fake Postgres (NewComposerWithMongo
...
func upstreamFakeMongo(colls map[string]*fakeColl) *MongoDB {
	return newFakeMongoFunc(func(name string) mongoColl {
...
func ordersBuyerView() *ViewDefinition {
	external := FromSchema(
		NewExternalSchema("users").PK("ID", "id").Field("Name", "name")).
		On("buyer_id").As("Buyer")
	return View("orders").Version(1).Root("orders").Schema(composerRootSchema()).
		Embed("buyer", external)
}

// ordersRootPool returns a fakePool whose composer query yields the order root
// row carrying the buyer_id FK that points at the upstream doc.
func ordersRootPool() *fakePool {
	p := newFakePool()
	p.queryHandler = func(sql string, args []any) (pgx.Rows, error) {
...
func newTestUpstream(t *testing.T, cfg UpstreamSubscriberConfig, mongo *MongoDB, pool *fakePool) *UpstreamSubscriber {
	t.Helper()
	if cfg.Topic == "" {
	...
	// composerPG drives Compose's root fetch via the pgxPool seam; the
	// subscriber's own pg handle is nil so the best-effort PG failure writers
	// no-op (their Pool() path is not unit-reachable — see file header note).
	composer := NewComposerWithMongo(newFakePostgres(pool), mongo)
...
func TestProcessMessage_Inserted_FullRipple(t *testing.T) {
	colls := happyColls()
	s := newTestUpstream(t, UpstreamSubscriberConfig{}, upstreamFakeMongo(colls), ordersRootPool())
...
func TestProcessMessage_Updated_WithFilter(t *testing.T) {
	...
	s := newTestUpstream(t, cfg, upstreamFakeMongo(colls), ordersRootPool())
	...
	set, _ := colls["users"].updates[0]["$set"].(bson.M)
...
func TestProcessMessage_Deleted_Cascade(t *testing.T) {
	...
	s := newTestUpstream(t, UpstreamSubscriberConfig{OnUpstreamDelete: upstreamDeletePolicyCascade},
		upstreamFakeMongo(colls), ordersRootPool())
...
func TestRipple_ComposeFailure(t *testing.T) {
	...
	s := newTestUpstream(t, UpstreamSubscriberConfig{}, upstreamFakeMongo(colls), pool)
...
func TestRipple_ComposeReturnsNilDoc(t *testing.T) {
	...
	pool := ordersRootPool()
	pool.queryHandler = func(sql string, args []any) (pgx.Rows, error) {
		return &composerRows{cols: []string{"id", "buyer_id", "name"}, data: nil}, nil
	}
	s := newTestUpstream(t, UpstreamSubscriberConfig{}, upstreamFakeMongo(colls), pool)
...
func TestRipple_NoJoinField(t *testing.T) {
	...
	other := FromSchema(NewExternalSchema("partners").PK("ID", "id")).On("partner_id").As("Partner")
	view := View("orders").Version(1).Root("orders").Schema(composerRootSchema()).Embed("partner", other)
	mongo := upstreamFakeMongo(happyColls())
	composer := NewComposerWithMongo(newFakePostgres(ordersRootPool()), mongo)
...
func TestDecodePayload(t *testing.T) {
	s := newTestUpstream(t, UpstreamSubscriberConfig{}, upstreamFakeMongo(happyColls()), ordersRootPool())
...
func TestApplyFilter(t *testing.T) {
	s := newTestUpstream(t, UpstreamSubscriberConfig{Filter: []string{"name"}},
		upstreamFakeMongo(happyColls()), ordersRootPool())
...
func TestJoinFieldFor(t *testing.T) {
	...
	other := View("x").Version(1).Root("orders").Schema(composerRootSchema()).
		Embed("partner", FromSchema(NewExternalSchema("partners").PK("ID", "id")).On("partner_id").As("Partner"))
...
func TestNewUpstreamSubscriber_Validation(t *testing.T) {
	mongo := upstreamFakeMongo(happyColls())
	pg := newFakePostgres(ordersRootPool())
...
func TestShutdown(t *testing.T) {
	...
	s2 := newTestUpstream(t, UpstreamSubscriberConfig{}, upstreamFakeMongo(happyColls()), ordersRootPool())
	s2.inflight.Add(1)
