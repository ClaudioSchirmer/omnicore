...
// its TableSchema + embed tree. Each level's columns are the schema's declared
...
func buildExportNode(schema *TableSchema, embeds []embedDef, goSegment, wireSegment string) *queries.ExportNode {
	...
	if schema != nil {
		...
		for _, f := range schema.fields {
			node.Columns = append(node.Columns, queries.ExportColumn{
				GoField:  f.goName,
				WireLeaf: domain.ToLowerCamel(f.goName),
				LabelKey: labels[f.goName],
			})
		}
	}
