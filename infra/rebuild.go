...
func (s *SyncEngine) ExecuteRebuild(ctx context.Context, plan DriftPlan, cfg RebuildConfig) error {
	...
	acquired, err := TryAcquireViewLock(ctx, conn, collection)
	if err != nil {
		return err
	}
	if !acquired {
		holder, _ := ReadViewLockHolder(ctx, conn, collection)
		if holder == nil {
			return fmt.Errorf("rebuild lock on view %q held by another connection (holder details unavailable) — another instance of the service is rebuilding this view", collection)
		}
		return fmt.Errorf(
			"rebuild lock on view %q held by pid=%d application=%q client=%q — another instance is rebuilding",
			collection, holder.PID, holder.ApplicationName, holder.ClientAddr)
	}

	// Step 3 — defer release. Lock auto-releases on conn.Release() too,
	// but explicit unlock surfaces errors via slog.
	defer func() {
		if releaseErr := ReleaseViewLock(ctx, conn, collection); releaseErr != nil {
	...
	if err := BeginRebuild(ctx, conn, collection, now); err != nil {
		return err
	}
	...
	if err := EndRebuild(ctx, conn, EndRebuildInput{
		ViewName:     collection,
...
func (s *SyncEngine) InitRegistryOnly(ctx context.Context, plan DriftPlan, serviceName string) error {
	return InitViewRegistry(ctx, s.pg.pool, InitViewRegistryInput{
...
func (s *SyncEngine) RefreshRegistryArtifactOnly(ctx context.Context, plan DriftPlan, serviceName string) error {
	return EndRebuild(ctx, s.pg.pool, EndRebuildInput{
		ViewName:     plan.View.Name(),
		Version:      plan.CurrentVersion,
		RebuildHash:  plan.CurrentRebuildHash,
		ArtifactHash: plan.CurrentArtifactHash,
...
func sampleExpectedFieldNames(ctx context.Context, c *Composer, view *ViewDefinition) (map[string]struct{}, error) {
	q := "SELECT id FROM " + validIdentifier(view.rootTable) + " LIMIT " + fmt.Sprintf("%d", sampleSizeForCleanup)
	rows, err := c.pg.pool.Query(ctx, q)
	if err != nil {
...
func (s *SyncEngine) rebuildFromTable(ctx context.Context, view *ViewDefinition, since string) error {
	...
	if since != "" {
		q = "SELECT id FROM " + validIdentifier(view.rootTable) + " WHERE updated_at >= $1 ORDER BY updated_at"
		args = []any{since}
	} else {
		q = "SELECT id FROM " + validIdentifier(view.rootTable) + " ORDER BY created_at"
	}
