...
func TestSQLConstants_ReferenceCorrectTable(t *testing.T) {
	cases := []struct {
		...
		{"sqlReadViewRegistry", sqlReadViewRegistry},
		{"sqlInitViewRegistry", sqlInitViewRegistry},
		{"sqlBeginRebuild", sqlBeginRebuild},
		{"sqlEndRebuild", sqlEndRebuild},
...
func TestEndRebuildSQL_CapturesPreviousState(t *testing.T) {
	// previous_* triple comes from the row's CURRENT state (Postgres reads
	...
	for _, expr := range []string{
		...
		if !strings.Contains(sqlEndRebuild, expr) {
			t.Errorf("sqlEndRebuild missing previous_* capture: %q", expr)
		}
	}
	if !strings.Contains(sqlEndRebuild, "status = 'done'") {
