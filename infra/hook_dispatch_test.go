...
//
//   - AdaptWriteOptions translates typed persistence.WriteOption[T] into
//     the type-erased writeHook the Postgres methods consume.
...
// hookFlatEntity is the test stand-in T for AdaptWriteOptions[*hookFlatEntity].
...
func TestAdaptWriteOptions_Empty(t *testing.T) {
	hook := AdaptWriteOptions[*hookFlatEntity](nil)
...
func TestAdaptWriteOptions_AfterBeginFires(t *testing.T) {
	...
	hook := AdaptWriteOptions[*hookFlatEntity]([]persistence.WriteOption[*hookFlatEntity]{
