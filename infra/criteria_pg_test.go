...
func TestPgVisitor_Operators(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			sql, args, err := compileWhere(c.e, r)
			if err != nil {
...
func TestPgVisitor_NestingAndPrecedence(t *testing.T) {
	...
	sql, args, err := compileWhere(e, testResolver())
	if err != nil {
		t.Fatalf("compileWhere: %v", err)
	}
...
func TestPgVisitor_NilWhereIsEmpty(t *testing.T) {
	sql, args, err := compileWhere(nil, testResolver())
...
func TestPgVisitor_UnknownFieldErrors(t *testing.T) {
	if _, _, err := compileWhere(criteria.Eq("Nope", "x"), testResolver()); err == nil {
		t.Fatal("expected unknown-field error")
	}
}

func TestPgVisitor_ZeroValueInErrors(t *testing.T) {
	if _, _, err := compileWhere(criteria.In("Name"), testResolver()); err == nil {
...
func TestPgVisitor_CardinalityErrors(t *testing.T) {
	...
	if _, _, err := compileWhere(criteria.Comparison{Field: "Name", Op: criteria.OpEq}, testResolver()); err == nil {
		t.Error("expected error: eq with zero values")
	}
	if _, _, err := compileWhere(criteria.Comparison{Field: "Phone", Op: criteria.OpIsNull, Values: []any{"x"}}, testResolver()); err == nil {
		t.Error("expected error: isnull with values")
	}
}

func TestPgVisitor_EmptyLogicalErrors(t *testing.T) {
	if _, _, err := compileWhere(criteria.And(), testResolver()); err == nil {
		t.Error("expected error: And with no operands")
	}
}

func TestPgVisitor_DomainIDArgUnwrapped(t *testing.T) {
	_, args, err := compileWhere(criteria.Eq("ID", domain.NewID("the-id")), testResolver())
	if err != nil {
		t.Fatalf("compileWhere: %v", err)
	}
...
func TestCompileOrder(t *testing.T) {
	r := testResolver()
	s, err := compileOrder([]criteria.OrderField{{Field: "Name"}, {Field: "Email", Desc: true}}, r)
	if err != nil {
	...
	if got, _ := compileOrder(nil, r); got != "" {
		t.Errorf("empty order = %q, want \"\"", got)
	}
	if _, err := compileOrder([]criteria.OrderField{{Field: "Nope"}}, r); err == nil {
