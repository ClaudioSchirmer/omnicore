...
// DetectViewDrift reads each view's PG registry row, computes the current
// hashes, and produces the per-view DriftPlan via the §9.1 decision
...
func DetectViewDrift(ctx context.Context, mongo *MongoDB, pg *Postgres, views []*ViewDefinition) (*DriftReport, error) {
	report := &DriftReport{Plans: make([]DriftPlan, 0, len(views))}
	exec := pg.pool
	for _, v := range views {
		registry, err := ReadViewRegistry(ctx, exec, v.Name())
		if err != nil {
			return nil, err
		}
		populated, err := hasUserDocuments(ctx, mongo, v.Name())
		if err != nil {
			return nil, err
		}
		plan := DriftPlan{
...
// hasUserDocuments returns true when the collection carries at least one
// document. Stops at the first match via SetLimit(1) so the query is O(1)
// even on collections of millions of docs. Mongo collections no longer
