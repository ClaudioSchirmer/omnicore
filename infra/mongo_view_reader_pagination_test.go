...
// Ensure the LimitExceededError implements NotificationCarrier (via the
// embedded *InfrastructureError). Defense against accidental breakage of the
// Pipeline contract.
func TestLimitExceededError_IsNotificationCarrier(t *testing.T) {
	var carrier domain.NotificationCarrier
	err := error(LimitExceededError(100))
	if !errors.As(err, &carrier) {
		t.Fatalf("LimitExceededError should implement NotificationCarrier")
	}
