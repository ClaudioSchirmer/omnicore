...
// fakePgExec captures the last Exec/Query/QueryRow invocation so the
// helpers' SQL + arg shape can be asserted without a real Postgres.
// Query / QueryRow are not exercised by the unit tests (List shape is
// covered by the §12 integration suite) but the methods are present so
// the type satisfies pgExec.
...
func (f *fakePgExec) Exec(_ context.Context, sql string, args ...any) (pgconn.CommandTag, error) {
	f.lastSQL = sql
	f.lastArgs = args
	return f.tag, f.execErr
}

func (f *fakePgExec) Query(_ context.Context, _ string, _ ...any) (pgx.Rows, error) {
	return nil, errors.New("not implemented in fakePgExec")
}

func (f *fakePgExec) QueryRow(_ context.Context, _ string, _ ...any) pgx.Row {
	return nil
}

func TestRecordUpstreamFailure_RejectsEmptyTopic(t *testing.T) {
	err := RecordUpstreamFailure(context.Background(), &fakePgExec{}, UpstreamFailureRecord{
		ViewName:   "orders",
		UpstreamID: "u1",
...
func TestRecordUpstreamFailure_PassesArgsInOrder(t *testing.T) {
	fake := &fakePgExec{}
	err := RecordUpstreamFailure(context.Background(), fake, UpstreamFailureRecord{
		SubscriptionTopic: "users.events",
		ViewName:          "orders",
		UpstreamID:        "u1",
		LocalID:           "ord-7",
...
func TestResolveUpstreamFailures_RejectsEmptyInputs(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			err := ResolveUpstreamFailures(context.Background(), &fakePgExec{},
...
func TestUpstreamSubscriber_RetryPendingFailures_NilPGFailsFast(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "no Postgres handle") {
		t.Errorf("expected no-PG-handle error, got %v", err)
	}
	if n != 0 {
