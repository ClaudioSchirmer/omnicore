...
func TestView_EmbedAddsOneToOneSource(t *testing.T) {
	v := View("v").Schema(rootSchema("v")).Embed(extLeg("child", "Child", "child")).On("v_id").Version(1)
...
// and still boot. Depth is capped structurally — a leg exposes no Embed/EmbedMany
// builder, so embed-of-embed is not expressible and fails to compile (no runtime
// guard, hence no negative test).
func TestValidateViewSchemas_ManyTopLevelEmbedsAllowed(t *testing.T) {
	...
	v := View("orders").Version(1).Schema(rootSchema("orders")).
		EmbedMany(extLeg("buyers", "Buyers", "buyers")).On("order_id").
		EmbedMany(extLeg("items", "Items", "items")).On("order_id").
		Embed(extLeg("owners", "Owner", "owner")).On("owner_id").
