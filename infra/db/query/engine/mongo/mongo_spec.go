...
// to the state each query.ViewDefinition asks for: creates absent collections
...
func buildCreateCollectionOptions(v *query.ViewDefinition) *options.CreateCollectionOptionsBuilder {
...
func applyIndexes(ctx context.Context, m *MongoDB, collection string, specs []*query.IndexSpec, forceRebuild bool) error {
	...
	for _, s := range specs {
		...
		if !forceRebuild {
			return fmt.Errorf("view %q: index %q diverges from declared spec; set %s=true to drop and recreate (driver error: %v)",
				collection, s.DerivedIndexName(), EnvForceRebuild, err)
		}
		// FORCE_REBUILD: drop the conflicting index and retry.
		idxName := s.DerivedIndexName()
...
func assertCollectionMatches(v *query.ViewDefinition, observed bson.M) error {
...
// collationDivergence compares the declared query.CollationSpec against the
...
// pronounced.
func collationDivergence(decl *query.CollationSpec, opts bson.M) string {
...
// cappedDivergence compares the declared query.CappedSpec against the
...
// is reported.
func cappedDivergence(decl *query.CappedSpec, opts bson.M) string {
...
// timeSeriesDivergence compares the declared query.TimeSeriesSpec against the
// observed timeseries sub-document.
func timeSeriesDivergence(decl *query.TimeSeriesSpec, opts bson.M) string {
