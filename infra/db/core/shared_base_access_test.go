...
func TestSharedBase_NaturalKeyRequiresColumn(t *testing.T) {
	assertPanics(t, "empty NaturalID column", func() {
		NewSharedBaseSchema("pessoa").Revision("revision").ID("id").Field("Name", "name").NaturalID("")
	})
}
...
func TestSharedBase_ScanPlanAbsent(t *testing.T) {
	...
	noLink := NewTableSchema[schemaSample]("aluno").ID("id").Field("Name", "name")
...
func TestAssertSharedBaseEquivalent_ChildNameDiverges(t *testing.T) {
	...
	a := declare().Child(NewTableSchema[embedFixture]("docs").ID("id").ParentID("pessoa_id"))
	b := declare().Child(NewTableSchema[schemaSample]("docs").ID("id").ParentID("pessoa_id"))
