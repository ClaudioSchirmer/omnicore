...
func TestInitViewRegistry_AndRead(t *testing.T) {
	...
	row, err := ReadViewRegistry(ctx, q, d, "users")
	if err != nil {
	...
	if err := InitViewRegistry(ctx, q, d, in); err != nil {
...
func TestBeginEndRebuild_StateMachine(t *testing.T) {
	...
	// Seed.
	if err := InitViewRegistry(ctx, q, d, InitViewRegistryInput{
	...
	// Begin.
	now := time.Now().UTC()
	if err := BeginRebuild(ctx, q, d, "users", now); err != nil {
		t.Fatalf("BeginRebuild: %v", err)
	}
	row, _ := ReadViewRegistry(ctx, q, d, "users")
	...
	// End.
	if err := EndRebuild(ctx, q, d, EndRebuildInput{
