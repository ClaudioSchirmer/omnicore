...
// upstream_failures_test.go and coverage_seams_test.go via a db.Querier fake.
...
func ordersBuyerView() *ViewDefinition {
	external := FromSchema(
		db.NewExternalSchema("users").PK("id").Field("Name", "name")).
...
func newTestUpstream(t *testing.T, cfg UpstreamSubscriberConfig, mongo *MongoDB, eng db.RelationalEngine) *UpstreamSubscriber {
	t.Helper()
	if cfg.Topic == "" {
...
func TestRipple_NoJoinField(t *testing.T) {
	...
	other := FromSchema(db.NewExternalSchema("partners").PK("id")).On("partner_id").As("Partner")
...
func TestJoinFieldFor(t *testing.T) {
	...
	other := View("x").Version(1).Root("orders").Schema(composerRootSchema()).
		Embed("partner", FromSchema(db.NewExternalSchema("partners").PK("id")).On("partner_id").As("Partner"))
