...
//
// Built once from the ViewDefinition's db.TableSchema tree; consumed by the
...
func newViewNode(schema *db.TableSchema, embeds []embedDef) *viewNode {
	n := &viewNode{
		schema:      schema,
...
// hasSchema reports whether this node carries a db.TableSchema. A registered view
