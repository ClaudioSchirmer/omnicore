			...
			for _, key := range s.KeyNames() {
				if _, ok := cols[key]; !ok {
					return fmt.Errorf("view %q: index #%d key %q is not a column the composer emits — "+
						"the index would never be used (typo, or a field not declared in the view's db.TableSchema). "+
...
func collectComposedColumns(schema *db.TableSchema, embeds []embedDef, prefix string, set map[string]struct{}) {
	add := func(col string) {
		if col == "" {
			return
		}
	...
	if schema != nil {
		add(schema.PKColumn())
		add(schema.FKColumn())
		sd, _ := schema.SoftDeleteColumn(); add(sd)
