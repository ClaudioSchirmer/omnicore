...
func TestApplyMongoSpecs_CreatesCollectionWithIndexes(t *testing.T) {
	...
	v := View("apply_users").Root("apply_users").Version(1).
		Indexes(
			Index("email").Unique(),
			Index("created_at").Desc(),
		)

	if err := ApplyMongoSpecs(context.Background(), m, []*ViewDefinition{v}); err != nil {
...
func TestApplyMongoSpecs_IsIdempotent(t *testing.T) {
	...
	v := View("apply_idem").Root("apply_idem").Version(1).
...
func TestApplyMongoSpecs_RejectsInvalidView(t *testing.T) {
	...
	v := View("invalid").Root("invalid")
	err := ApplyMongoSpecs(context.Background(), m, []*ViewDefinition{v})
	if err == nil {
...
func TestApplyMongoSpecs_CreatesValidatorOnFreshCollection(t *testing.T) {
	...
	v := View("apply_validator").Root("apply_validator").Version(1).
		JSONSchema(schema)
...
func TestCheckServiceRegistry_HappyPathAndIdempotent(t *testing.T) {
	...
	v := View("my_view").Root("my_view").Version(1)
	if err := ApplyMongoSpecs(context.Background(), m, []*ViewDefinition{v}); err != nil {
		t.Fatalf("apply: %v", err)
	}

	// First call writes the marker.
	if err := CheckServiceRegistry(context.Background(), m, "svc-a", "prd", []*ViewDefinition{v}); err != nil {
...
func TestCheckServiceRegistry_DevDowngradesForeignToWarn(t *testing.T) {
	...
	v := View("declared").Root("declared").Version(1)
	if err := ApplyMongoSpecs(context.Background(), m, []*ViewDefinition{v}); err != nil {
		t.Fatalf("apply: %v", err)
	}

	// dev → warn, no error.
	if err := CheckServiceRegistry(context.Background(), m, "svc", "dev", []*ViewDefinition{v}); err != nil {
...
func TestCheckServiceRegistry_NonDevAbortsOnForeign(t *testing.T) {
	...
	err := CheckServiceRegistry(context.Background(), m, "svc", "prd", []*ViewDefinition{v})
	if err == nil {
...
func TestDetectViewDrift_FreshInit(t *testing.T) {
	...
	v := View("drift_users").Root("drift_users").Version(1)
	// No registry row, no Mongo docs → FreshInit.
	report, err := DetectViewDrift(context.Background(), m, pg, []*ViewDefinition{v})
	if err != nil {
...
func TestDetectViewDrift_NoneAndArtifactOnlyAndAlienData(t *testing.T) {
	...
	v := View("drift_x").Root("drift_x").Version(1).
		Indexes(Index("email").Unique())
	...
	if err := InitViewRegistry(context.Background(), pg.Querier(), pg.Dialect(), in); err != nil {
		t.Fatalf("Init: %v", err)
	}
	report, _ = DetectViewDrift(context.Background(), m, pg, []*ViewDefinition{v})
	if report.Plans[0].Decision != DriftNone {
