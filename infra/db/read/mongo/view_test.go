...
func pgEmbed(table, fk string) *Source {
	return FromSchema(db.NewTableSchema[embedFixture](table).PK("id").FK(fk))
}

func mongoEmbed(table, fk string) *Source {
	return FromSchema(db.NewExternalSchema(table).PK("id").FK(fk))
}
...
func rootSchema(table string) *db.TableSchema {
	return db.NewTableSchema[embedFixture](table).PK("id").SoftDelete("deleted_at")
}
...
// whose db.TableSchema carries Child(...) is a fatal view-validation error — views
...
func TestValidateViewSchemas_RejectsEmbedSourceWithChildren(t *testing.T) {
	grand := db.NewTableSchema[embedFixture]("tags").PK("id").FK("address_id")
	src := FromSchema(db.NewTableSchema[embedFixture]("addresses").
...
func TestValidateViewSchemas_RootSchemaWithChildrenOK(t *testing.T) {
	rootWithChild := db.NewTableSchema[embedFixture]("users").PK("id").SoftDelete("deleted_at").
		Child(db.NewTableSchema[schemaSample]("addresses").PK("id").FK("user_id"))
	v := View("users").Version(1).Root("users").
...
func TestValidateViewSchemas_RejectsRootWithoutPK(t *testing.T) {
	v := View("users").Version(1).Root("users").
		Schema(db.NewTableSchema[embedFixture]("users").SoftDelete("deleted_at")). // no .PK
		EmbedMany("addresses", pgEmbed("addresses", "user_id"))
	err := ValidateViewSchemas([]*ViewDefinition{v})
...
func TestValidateViewSchemas_RejectsEmbedSourceWithoutPK(t *testing.T) {
	src := FromSchema(db.NewTableSchema[embedFixture]("addresses").FK("user_id")) // no .PK
	v := View("users").Version(1).Root("users").
		Schema(rootSchema("users")).
		EmbedMany("addresses", src)
	err := ValidateViewSchemas([]*ViewDefinition{v})
...
func TestValidateViewSchemas_RejectsEmbedManyWithoutFK(t *testing.T) {
	src := FromSchema(db.NewTableSchema[embedFixture]("addresses").PK("id")) // no .FK
	v := View("users").Version(1).Root("users").
		Schema(rootSchema("users")).
		EmbedMany("addresses", src)
	err := ValidateViewSchemas([]*ViewDefinition{v})
...
func TestValidateViewSchemas_RejectsOneToOneEmbedWithoutOn(t *testing.T) {
	src := FromSchema(db.NewTableSchema[embedFixture]("buyer").PK("id")) // no .On
...
func TestSource_SchemaDef_AndKindFromSchema(t *testing.T) {
	ext := db.NewExternalSchema("users").PK("id")
	mongo := FromSchema(ext)
	if mongo.SchemaDef() != ext {
		t.Error("SchemaDef() must return the schema FromSchema was built with")
	}
	if !mongo.IsMongo() {
		t.Error("FromSchema(db.NewExternalSchema(...)) must be a Mongo source")
	}
	pg := FromSchema(db.NewTableSchema[embedFixture]("addresses").PK("id"))
	if pg.IsMongo() {
		t.Error("FromSchema(db.NewTableSchema[...]) must be a PG source")
	}
...
func TestViewNode_TranslatesGoPathToColumnAndBack(t *testing.T) {
	rootSchema := db.NewTableSchema[vsRoot]("people").
		PK("person_pk").
		Field("Email", "mail").
		SoftDelete("removed_at").
		CreatedAt("created_at")
	childSchema := db.NewExternalSchema("tags").
		PK("tag_pk").
		FK("person_ref").
		Field("ZipCode", "zip")
