...
// ---------------------------------------------------------------------------
// db.LimitExceededError — shape of the typed 400 envelope produced when ?limit=
// exceeds the resolved ceiling.
// ---------------------------------------------------------------------------

func TestLimitExceededError_CarriesSchemaContextAndFieldValue(t *testing.T) {
	err := db.LimitExceededError(250)
	if err == nil {
...
func TestViewDefinition_MaxLimit_DoesNotChangeRebuildHash(t *testing.T) {
	base := View("users").Version(1).Root("users")
	bumped := View("users").Version(1).Root("users").MaxLimit(500)
...
func TestViewDefinition_MaxLimit_PreservesValue(t *testing.T) {
	v := View("users").MaxLimit(500)
	if v.MaxLimitValue() != 500 {
		t.Fatalf("want 500, got %d", v.MaxLimitValue())
	}
}

// Ensure the db.LimitExceededError implements NotificationCarrier (via the
// embedded *db.InfrastructureError). Defense against accidental breakage of the
// Pipeline contract.
func TestLimitExceededError_IsNotificationCarrier(t *testing.T) {
	var carrier domain.NotificationCarrier
	err := error(db.LimitExceededError(100))
	if !errors.As(err, &carrier) {
		t.Fatalf("db.LimitExceededError should implement NotificationCarrier")
	}
