...
func (r *MongoViewReader) ReadPage(ctx context.Context, view string, c queries.ReadCriteria) (queries.Page, error) {
	...
	if !c.BypassMaxLimit && c.Limit > maxLimit {
		return queries.Page{}, db.LimitExceededError(maxLimit)
	}
	limit := c.Limit
	if limit <= 0 {
		limit = maxLimit
	...
	if c.After != "" || c.Before != "" {
		...
		if decErr != nil {
			return queries.Page{}, db.InvalidCursorError(fmt.Errorf("invalid cursor: %w", decErr))
		}
		if len(cursor.K)-1 != len(c.Sort) {
			return queries.Page{}, db.InvalidCursorError(fmt.Errorf("cursor tuple length %d does not match sort field count %d",
		...
		if cursor.H != queries.HashContext(c.Filter, c.Sort, c.Search, c.IncludeArchived) {
			return queries.Page{}, db.InvalidCursorError(errors.New("cursor context hash does not match current criteria"))
		}
		direction := 1
		if backward {
			direction = -1
