...
// --- db.BaseAggregateRepository.FindByID / FindArchivedByID ------------------
...
func TestBaseAggregateRepository_FindByID(t *testing.T) {
	...
	bar := db.NewBaseAggregateRepository[*loaderRoot](pg, func() *loaderRoot { return &loaderRoot{} })
	// loaderRoot.AggregateChildren() declares both children, and a write-capable
	// db.BaseAggregateRepository enforces aggregate-boundary agreement, so the full
...
// --- db.InfrastructureError helpers ----------------------------------------

func TestNewInfrastructureError(t *testing.T) {
	ctxA := domain.NewNotificationContext("A")
	e := db.NewInfrastructureError([]*domain.NotificationContext{ctxA})
	if e == nil || len(e.Contexts) != 1 {
		t.Errorf("db.NewInfrastructureError = %+v", e)
	}
}

func TestInfrastructureError_ErrorMessage(t *testing.T) {
	e := db.NewInfrastructureError([]*domain.NotificationContext{
...
func TestInfrastructureError_SatisfiesCarrier(t *testing.T) {
	var carrier domain.NotificationCarrier = db.NewInfrastructureError(nil)
	if carrier == nil {
...
func TestInfrastructureSingleNotificationError(t *testing.T) {
	e := db.SingleNotificationError("X", "f", domain.RequiredFieldNotification{})
	if e == nil || len(e.Contexts) != 1 {
		t.Fatalf("%+v", e)
	}
...
func TestInfrastructureFieldErrorWithCause(t *testing.T) {
	...
	e := db.FieldErrorWithCause("X", "f", cause, domain.RequiredFieldNotification{})
