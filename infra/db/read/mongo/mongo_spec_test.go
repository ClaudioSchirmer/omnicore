...
func TestDeriveIndexName_SingleAscending(t *testing.T) {
	got := deriveIndexName(Index("email"))
	if got != "email_1" {
		t.Errorf("deriveIndexName = %q, want %q", got, "email_1")
	}
}

func TestDeriveIndexName_Descending(t *testing.T) {
	got := deriveIndexName(Index("created_at").Desc())
	if got != "created_at_-1" {
		t.Errorf("deriveIndexName = %q, want %q", got, "created_at_-1")
	}
}

func TestDeriveIndexName_Compound(t *testing.T) {
	got := deriveIndexName(Compound("email", "created_at").Desc())
	if got != "email_1_created_at_-1" {
		t.Errorf("deriveIndexName = %q, want %q", got, "email_1_created_at_-1")
	}
}

func TestDeriveIndexName_Text(t *testing.T) {
	got := deriveIndexName(TextIndex("name", "email"))
	if got != "name_text_email_text" {
		t.Errorf("deriveIndexName = %q, want %q", got, "name_text_email_text")
	}
}

func TestDeriveIndexName_GeoAndHashed(t *testing.T) {
	if got := deriveIndexName(GeoIndex("location")); got != "location_2dsphere" {
		t.Errorf("geo deriveIndexName = %q", got)
	}
	if got := deriveIndexName(Index("user_id").Hashed()); got != "user_id_hashed" {
		t.Errorf("hashed deriveIndexName = %q", got)
	}
}

func TestDeriveIndexName_ExplicitNameWins(t *testing.T) {
	got := deriveIndexName(Index("email").Name("custom_idx"))
	if got != "custom_idx" {
...
func TestBuildValidatorCommand_DefaultLevelAction(t *testing.T) {
	...
	if cmd[2].Key != "validationLevel" || cmd[2].Value != ValidationLevelStrict {
		t.Errorf("cmd[2] = %+v, want {validationLevel, strict}", cmd[2])
	}
	if cmd[3].Key != "validationAction" || cmd[3].Value != ValidationActionError {
...
func TestBuildValidatorCommand_OverrideLevelAndAction(t *testing.T) {
	...
	cmd := buildValidatorCommand(v)
	if cmd[2].Value != ValidationLevelModerate {
		t.Errorf("validationLevel = %v, want %q", cmd[2].Value, ValidationLevelModerate)
	}
	if cmd[3].Value != ValidationActionWarn {
		t.Errorf("validationAction = %v, want %q", cmd[3].Value, ValidationActionWarn)
	}
}

func TestBuildValidatorCommand_PayloadWrapsJSONSchema(t *testing.T) {
	schema := bson.M{"required": []string{"_id", "name"}}
	v := View("users").Root("users").JSONSchema(schema)
	cmd := buildValidatorCommand(v)
...
func TestCollationDivergence_DeclaredButAbsent(t *testing.T) {
	diag := collationDivergence(&CollationSpec{Locale: "pt"}, bson.M{})
...
func TestCollationDivergence_StrengthMismatch(t *testing.T) {
	diag := collationDivergence(
		&CollationSpec{Locale: "pt", Strength: 1},
...
func TestCappedDivergence_DeclaredButUncapped(t *testing.T) {
	diag := cappedDivergence(&CappedSpec{SizeBytes: 1024}, bson.M{})
...
func TestCappedDivergence_MaxDocsMismatch(t *testing.T) {
	diag := cappedDivergence(
		&CappedSpec{SizeBytes: 1024, MaxDocs: 100},
...
func TestTimeSeriesDivergence_DeclaredButPlainCollection(t *testing.T) {
	diag := timeSeriesDivergence(&TimeSeriesSpec{TimeField: "ts"}, bson.M{})
...
func TestTimeSeriesDivergence_GranularityMismatch(t *testing.T) {
	diag := timeSeriesDivergence(
		&TimeSeriesSpec{TimeField: "ts", Granularity: "seconds"},
...
func TestAssertCollectionMatches_PropagatesCollationDiag(t *testing.T) {
	v := View("users").Root("users").
		Collation(&CollationSpec{Locale: "pt"})
	observed := bson.M{"options": bson.M{
...
func TestAssertCollectionMatches_PropagatesCappedDiag(t *testing.T) {
	v := View("audit").Root("audit").Capped(&CappedSpec{SizeBytes: 1 << 20})
	observed := bson.M{"options": bson.M{
