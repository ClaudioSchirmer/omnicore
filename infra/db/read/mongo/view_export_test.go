...
func buildExportTestView() *ViewDefinition {
	userSchema := db.NewTableSchema[*expUser]("users").
		PK("id").
		Field("Name", "name").
		Field("Email", "email")
	addrSchema := db.NewTableSchema[expAddr]("addresses").
		PK("id").
		FK("user_id").
		Field("ZipCode", "zip_code")
	// External (type-less) source carries its labelKey inline — the "mini-domain".
	partnerSchema := db.NewExternalSchema("partners").
		PK("id").
...
func TestField_SchemaLabelExternalOnly(t *testing.T) {
	...
	db.NewExternalSchema("p").Field("Name", "name", "K")
	...
	assertPanics(t, "type-anchored schema-level label", func() {
		db.NewTableSchema[*expUser]("users").Field("Name", "name", "K")
	})

	// More than one label panics.
	assertPanics(t, "two labels", func() {
		db.NewExternalSchema("p").Field("Name", "name", "A", "B")
	})
}
