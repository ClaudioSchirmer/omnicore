...
// db.BaseRepository.Scope (which takes the concrete *AppContext).
func testCtx() *configuration.AppContext {
	return configuration.NewAppContextWithRandomID(configuration.LangENG)
}

// noHook is the zero-value db.WriteHook the integration tests pass when
// they exercise the persister without lifecycle hook firing — same as
// "no opts on the db.BaseRepository call site".
