...
func TestRebuildView_RebuildsFromTable(t *testing.T) {
	...
	view := View("rv_users").Root("rv_users").Schema(rootSchema("rv_users")).Version(1)
...
func TestRebuildAllViews(t *testing.T) {
	...
	va := View("rv_a").Root("rv_a").Schema(rootSchema("rv_a")).Version(1)
	vb := View("rv_b").Root("rv_b").Schema(rootSchema("rv_b")).Version(1)
...
func TestRebuildViewSince_FiltersByUpdatedAt(t *testing.T) {
	...
	view := View("rv_since").Root("rv_since").
...
func TestExecuteRebuild_HappyPath(t *testing.T) {
	...
	view := View("er_users").Root("er_users").Schema(rootSchema("er_users")).Version(1)

	// Seed the registry row at the SAME hash so EndRebuild can find it.
	now := time.Now().UTC()
	InitViewRegistry(context.Background(), pg.Querier(), pg.Dialect(), InitViewRegistryInput{
	...
	row, _ := ReadViewRegistry(context.Background(), pg.Querier(), pg.Dialect(), view.Name())
...
func TestExecuteRebuild_RejectsInvalidOrphanMode(t *testing.T) {
	...
	view := View("er_x").Root("er_x").Schema(rootSchema("er_x")).Version(1)
...
func TestInitRegistryOnly_WritesRow(t *testing.T) {
	...
	view := View("ir_users").Root("ir_users").Schema(rootSchema("ir_users")).Version(1)
	engine := NewSyncEngine(pg, m, nil, "", []*ViewDefinition{view}, 1)
	plan := DriftPlan{
		View:                view,
		CurrentVersion:      view.VersionNumber(),
	...
	if row, _ := ReadViewRegistry(context.Background(), pg.Querier(), pg.Dialect(), view.Name()); row == nil {
...
func TestRefreshRegistryArtifactOnly(t *testing.T) {
	...
	view := View("rfa").Root("rfa").Schema(rootSchema("rfa")).Version(1)
	// Seed.
	now := time.Now().UTC()
	InitViewRegistry(context.Background(), pg.Querier(), pg.Dialect(), InitViewRegistryInput{
