...
//
//   - db.AdaptWriteOptions translates typed persistence.WriteOption[T] into
//     the type-erased db.WriteHook the pg.Postgres methods consume.
...
// hookFlatEntity is the test stand-in T for db.AdaptWriteOptions[*hookFlatEntity].
...
func TestAdaptWriteOptions_Empty(t *testing.T) {
	hook := db.AdaptWriteOptions[*hookFlatEntity](nil)
...
func TestAdaptWriteOptions_AfterBeginFires(t *testing.T) {
	...
	hook := db.AdaptWriteOptions[*hookFlatEntity]([]persistence.WriteOption[*hookFlatEntity]{
