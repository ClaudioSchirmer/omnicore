...
func rebuildSyncEngine(eng db.RelationalEngine, coll mongoColl, views []*ViewDefinition) *SyncEngine {
...
func emptyRowsEngine() *fakeEngine {
	return newFakeEngine(&fakeQuerier{
		queryFn: func(string, []any) (db.Rows, error) { return &fakeRows{rows: 0}, nil },
	})
}
...
func TestRebuildAllViews_EmptyTables(t *testing.T) {
	...
	external := FromSchema(db.NewExternalSchema("buyers").PK("id").FK("order_id")).As("Buyers")
