...
// Schema attaches the view's root db.TableSchema (Go↔column + PK + soft-delete) —
...
// and read agree.
func (v *ViewDefinition) Schema(ts *db.TableSchema) *ViewDefinition {
	v.schema = ts
	return v
}

// SchemaDef returns the view's root db.TableSchema (nil when unset).
func (v *ViewDefinition) SchemaDef() *db.TableSchema { return v.schema }
...
// FromSchema produces an embed source from a db.TableSchema — the single,
// mandatory source constructor. Everything physical is derived from the schema:
//
//   - the table / collection is ts.Table();
//   - the source kind is the schema's kind — a type-less db.NewExternalSchema
...
func FromSchema(ts *db.TableSchema) *Source {
...
// SchemaDef returns the embed source's db.TableSchema (always set — FromSchema is
// the only constructor). Symmetric with ViewDefinition.SchemaDef().
func (s *Source) SchemaDef() *db.TableSchema { return s.schema }
...
func ValidateViewSchemas(views []*ViewDefinition) error {
	...
	return fmt.Errorf(
		"view schema(s) incomplete — every view (root + every embed) must declare a db.TableSchema, "+
...
func appendEmbedSchemaProblems(acc []string, viewName string, embeds []embedDef) []string {
	for _, e := range embeds {
		...
		if e.source.schema == nil {
			...
			// Grandchild-via-schema: an embed source whose db.TableSchema carries
			...
			if e.source.schema.HasChildren() {
				acc = append(acc, fmt.Sprintf(
					"view %q: embed %q (source %q) declares Child(...) on its db.TableSchema — "+
...
// view embedding FromSchema(db.NewExternalSchema("users")). The split keeps the namespaces separate:
//
