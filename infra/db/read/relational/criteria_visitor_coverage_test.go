...
func TestPgVisitor_NotErrors(t *testing.T) {
	...
	if _, _, err := compileWhere(criteria.Not(nil), testResolver(), testPGDialect{}); err == nil {
		t.Error("expected error: NOT with nil inner")
	}
	// NOT propagates an inner error (unknown field).
	if _, _, err := compileWhere(criteria.Not(criteria.Eq("Nope", "x")), testResolver(), testPGDialect{}); err == nil {
		t.Error("expected NOT to propagate inner unknown-field error")
	}
}

func TestPgVisitor_LogicalInnerErrorPropagates(t *testing.T) {
	if _, _, err := compileWhere(criteria.And(criteria.Eq("Name", "ok"), criteria.Eq("Nope", "x")), testResolver(), testPGDialect{}); err == nil {
		t.Error("expected AND to propagate inner unknown-field error")
	}
	// OR joiner branch.
	sql, _, err := compileWhere(criteria.Or(criteria.Eq("Name", "a"), criteria.Eq("Email", "b")), testResolver(), testPGDialect{})
...
func TestPgVisitor_UnsupportedOperator(t *testing.T) {
	...
	if _, _, err := compileWhere(bad, testResolver(), testPGDialect{}); err == nil {
