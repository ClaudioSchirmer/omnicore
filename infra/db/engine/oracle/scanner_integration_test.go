...
// A MANUAL root scanner runs on Oracle: it receives the backend-neutral
// core.Row and the loader runs it through the engine's Querier, so it works on
// any engine — the consumer owns the dialect-specific column decoding (here,
// RAW(16) → uuid string).
//
//	go test -tags=integration,oracle ./infra/db/engine/oracle/ -run ManualRootScanner -count=1
...
func TestOracleLoader_ManualRootScanner(t *testing.T) {
	eng, raw := setup(t)
	...
	id := uuid.New()
	if _, err := raw.ExecContext(ctx, `INSERT INTO scan_probe (id, label) VALUES (:1, :2)`, id[:], "manual"); err != nil {
		t.Fatalf("insert probe: %v", err)
	}

	// Manual root scanner: receives core.Row, decodes the RAW(16) id to a
