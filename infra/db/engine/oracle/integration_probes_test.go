...
// REAL Oracle: what the unit fakes cannot certify — the concrete forms go-ora
// delivers for SUM/AVG/MIN/MAX (every NUMBER arrives as text and must land
// back exact; AVG over BINARY_DOUBLE stays float64), NULL-on-empty-set
// scanning, the scope gate over live rows, the FETCH FIRST-rendered existence
// probes, and Ne("ID") encoding a uuid string against a RAW(16) primary key
// end to end.
...
func TestProbes_Oracle_ExistsScopeAndExcludeSelf(t *testing.T) {
	...
	// Ne("ID", <uuid string>) against a RAW(16) PK — the encode path that a
...
func TestProbes_Oracle_AggregateDSL(t *testing.T) {
	eng, _ := probeSetup(t)
	l := probeLoader(eng)
	ctx := context.Background()

	// Every fact in ONE SELECT. go-ora hands every NUMBER back as text — the
	// typed aggregate scan must land the minor-unit arithmetic back exact, and
	// AVG over BINARY_DOUBLE stays float64.
	total := read.Count()
	...
	if err := l.Aggregate(ctx, nil, read.SumInt("Area")); err == nil {
		t.Fatal("SumInt over BINARY_DOUBLE must error loudly (use Sum)")
	}
}

func TestProbes_Oracle_AggregateBy(t *testing.T) {
	eng, _ := probeSetup(t)
	l := probeLoader(eng)
	ctx := context.Background()
