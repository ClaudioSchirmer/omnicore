...
// The Oracle engine writes the in-TX audit_events row and publishes domain
// events — the mirror of the other engines' paths. Proves the dialect-rendered
// audit INSERT (:N placeholders, Go-generated row id, text-bound JSON payload
// into the native JSON column) executes on a real Oracle and that the
// post-commit echo + publisher fire.
//
//	go test -tags=integration,oracle ./infra/db/engine/oracle/ -run AuditAndEvents -count=1
...
func createAuditTable(t *testing.T, raw *sql.DB) {
	...
	// Mirrors the oracle framework migration's audit_events (the columns
...
func TestOracleEngine_AuditAndEvents(t *testing.T) {
	eng, raw := setup(t)
	...
	if err := raw.QueryRow(
		`SELECT COUNT(*), MAX(verb), MAX(entity_type), MAX(kind) FROM audit_events WHERE aggregate_id = :1`, res.ID.Value(),
	...
	if err := raw.QueryRow(`SELECT COUNT(*) FROM audit_events WHERE aggregate_id = :1`, res2.ID.Value()).Scan(&n); err != nil {
