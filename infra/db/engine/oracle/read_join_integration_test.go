...
// is dialect-shaped: the RAW(16) id codec the join's ON has to encode both ways,
// the OFFSET…FETCH row window, and — on Oracle above all — the IDENTIFIER LENGTH,
// since every traversal is rendered under a `j_<fk>` alias derived from the
...
func rjSetup(t *testing.T) *read.AggregateLoader[*rjOrder] {
	...
	// No teardown: newTestOracleSchema hands each test a throw-away schema and
	// drops it whole, so the fixture tables go with it.
	...
	for _, s := range []struct {
		...
		{`INSERT INTO rj_customers (id, nome, credito, owner_id) VALUES (:1, 'ana', 10, :2)`,
			[]any{ana[:], rjOwnerBytes(rjAnaOwner)}},
		{`INSERT INTO rj_customers (id, nome, credito, owner_id) VALUES (:1, 'bruno', 32, :2)`,
			[]any{bruno[:], rjOwnerBytes(rjBrunoOwner)}},
		{`INSERT INTO rj_carriers (id, codigo, credito, owner_id) VALUES (:1, 'DHL', 99, :2)`,
...
func TestOracleReadJoin_LoadsAcrossBothKinds(t *testing.T) {
	loader := rjSetup(t)
...
func TestOracleReadJoin_IsAddressableInACriteria(t *testing.T) {
	loader := rjSetup(t)
	ctx := context.Background()
...
func TestOracleReadJoin_ServedThroughARelationalView(t *testing.T) {
	loader := rjSetup(t)
	ctx := context.Background()
...
// rjOwnerBytes is the stored form of a deterministic owner id — RAW(16) on this
...
func TestOracleReadJoin_IdentityColumnDecodesTheStoredIdForm(t *testing.T) {
	loader := rjSetup(t)
...
func TestOracleReadJoin_IdentityColumnIsAddressableInACriteria(t *testing.T) {
	loader := rjSetup(t)
	ctx := context.Background()
