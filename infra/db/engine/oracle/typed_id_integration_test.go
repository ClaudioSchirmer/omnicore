...
// The type-driven identity contract on Oracle — the field's Go TYPE is the
// declaration, nothing is configured:
//
//   - domain.ID / *domain.ID field  → RAW(16) column: binds 16 bytes on write
//     and in lifted criteria probes; the id scan proxy restores the canonical
//     value (nil for SQL NULL on the pointer field).
//   - string / *string field → VARCHAR2(36) column: text in, text out, text
//     probes — never coerced, whatever the value's shape.
//
// TestOracleEngine_SecondaryUUIDColumn covers the required domain.ID quadrant;
// these two cover the nullable *domain.ID and the string/text quadrants. The
// Postgres/MySQL/SQL Server twins live in their engine packages.

// linkEntity: a NULLABLE identity reference (*domain.ID) over RAW(16) NULL.
...
func TestOracleEngine_NullableTypedIDColumn(t *testing.T) {
	eng, raw := setup(t)
	ctx := ctxFor()
	...
	if err != nil {
		t.Fatalf("Insert (non-nil *domain.ID into RAW(16)): %v", err)
	}
	...
	if len(rawPartner) != 16 {
		t.Fatalf("partner_id stored as %d bytes, want RAW(16)", len(rawPartner))
	}
	...
	if got.PartnerID == nil || got.PartnerID.Value() != partner.Value() {
		t.Fatalf("PartnerID = %v, want &%q (RAW(16) not restored into *domain.ID)", got.PartnerID, partner.Value())
	}
...
// the text choice. They pair with VARCHAR2(36) columns and are never coerced,
...
func TestOracleEngine_StringFieldsStayText(t *testing.T) {
	eng, raw := setup(t)
	ctx := ctxFor()
