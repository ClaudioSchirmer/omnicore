...
func TestBaseRepository_ConstraintCodeOtherThan23505ReturnsRaw(t *testing.T) {
	pg, cleanup := newTestPG(t)
	defer cleanup()
	// A table with a NOT NULL column the schema does not populate, so the
	// framework's INSERT (id + name + email + phone + managed) omits it and the
	// database raises a non-23505 error (23502 not_null_violation) — the
	// comment's "NULL into NOT NULL" vehicle. mapErr must return it RAW: the code
	// is not a unique violation, so it never matches a ConstraintBinding and is
	// not wrapped into a NotificationCarrier.
	createTable(t, pg, `CREATE TABLE flat_persons_nn (
		id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
		name TEXT NOT NULL,
		email TEXT NOT NULL,
		phone TEXT,
	...
	_, err = repo.Scope(testCtx()).Insert(ins)
	if err == nil {
		t.Fatal("expected a NOT NULL (23502) error inserting without the mandatory column")
	}
	// A non-23505 error is returned raw — not wrapped as a bound-constraint carrier.
	var carrier domain.NotificationCarrier
	if errorsAs(err, &carrier) {
		t.Errorf("non-23505 error must be returned raw, got carrier %T", err)
	}
}
...
// (effectiveContextName unit tests moved to infra/db — they exercise a db
// internal and need no live database.)
