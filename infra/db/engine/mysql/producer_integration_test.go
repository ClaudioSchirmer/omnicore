...
func createIntegrationEventsTable(t *testing.T, raw *sql.DB) {
	...
	if _, err := raw.ExecContext(ctx, `DROP TABLE IF EXISTS integration_events`); err != nil {
	...
	t.Cleanup(func() { _, _ = raw.ExecContext(context.Background(), `DROP TABLE IF EXISTS integration_events`) })
}

func TestMySQLProducer_ProducerDispatch(t *testing.T) {
	eng, raw := setup(t)
	...
	// would have taken it too): assert the payload landed verbatim.
	var payload string
	if err := raw.QueryRow(
		`SELECT payload FROM integration_events WHERE JSON_EXTRACT(payload, '$.path') = 'in-tx'`,
