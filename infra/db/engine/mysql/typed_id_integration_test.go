...
// The type-driven identity contract on MySQL — the field's Go TYPE is the
...
//
// TestMySQLEngine_SecondaryUUIDColumn covers the required domain.ID quadrant;
// these two cover the nullable *domain.ID and the string/text quadrants. The
// Postgres twins live in the postgres package (typed_id_integration_test.go).
...
func TestMySQLEngine_NullableTypedIDColumn(t *testing.T) {
	eng, raw := setup(t)
	ctx := ctxFor()
...
func TestMySQLEngine_StringFieldsStayText(t *testing.T) {
	eng, raw := setup(t)
	ctx := ctxFor()
