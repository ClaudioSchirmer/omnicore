...
// the neutral db.UnwrapTx bridge). The persister builds one per lifecycle-hook
...
// The canonical, backend-neutral bridge is db.UnwrapTx. Panics on a foreign
...
// NeutralTx exposes the live pgx.Tx through the backend-neutral db.Tx surface —
// the Postgres side of the db.UnwrapTx bridge. Exported so db.UnwrapTx (which
// type-asserts the neutralTxCarrier interface) can reach it on a sealed handle.
func (h *pgxTxHandle) NeutralTx() db.Tx { return pgTx{tx: h.tx} }

// pgTx adapts a live pgx.Tx to db.WriteTx: Exec drops the command tag, ExecCount
// reads it for the rows-affected count, Query wraps pgx.Rows as db.Rows (pgRows
// swallows the no-error Close), Commit/Rollback drive the lifecycle, Handle seals
// the tx for lifecycle hooks, and db.Dialect is the Postgres flavor.
type pgTx struct{ tx pgx.Tx }

// Compile-time proof pgTx satisfies the neutral write surface.
var _ db.WriteTx = pgTx{}
...
// lifecycle hooks receive (and recover via db.UnwrapTx / UnwrapPgxTx).
func (t pgTx) Handle() persistence.TxHandle { return newPgxTxHandle(t.tx) }

func (t pgTx) Dialect() db.Dialect { return pgDialect{} }
