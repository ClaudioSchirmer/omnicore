...
// Integration tests for the SQL Server engine's synchronous write path against
// a real SQL Server container (the QA bench's `sqlserver` profile service,
// host :14333, sa password OmnicoreQA!2026 by convention).
//
//	go test -tags=integration,sqlserver ./infra/db/engine/sqlserver/ -count=1
//
// Each test creates a THROW-AWAY database (like the pg/mysql harnesses) so
// repeat runs never stomp on the example service — that requires an
// admin-capable connection (sa); override it via
// OMNICORE_TEST_SQLSERVER_ADMIN_DSN (e.g. to point at a remote bench host).
// Verifies each write verb persists the row (UUID v7 round-tripping through
// BINARY(16)) and lands the matching outbox row in the same TX.

const defaultSQLServerAdminDSN = "server=127.0.0.1;port=14333;user id=sa;password=OmnicoreQA!2026;encrypt=true;TrustServerCertificate=true"

func sqlserverAdminDSN() string {
	if v := os.Getenv("OMNICORE_TEST_SQLSERVER_ADMIN_DSN"); v != "" {
		return v
	}
	return defaultSQLServerAdminDSN
}

// testDSN is the DSN of the current test's throw-away database, set by
// newTestSQLServerDB. The tests in this package do not use t.Parallel, so the
...
// newTestSQLServerDB creates a throw-away database on the bench SQL Server,
// points testDSN at it and registers its DROP as cleanup (LIFO: callers' own
// cleanups — closing engines/connections — run first; SINGLE_USER kicks any
// straggler session so the DROP never hangs). Skips the test when SQL Server
// is unreachable. The admin DSN must carry no `database=` param — the test
// database is appended to it.
func newTestSQLServerDB(t *testing.T) string {
	t.Helper()
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	admin, err := sql.Open("sqlserver", sqlserverAdminDSN())
	if err != nil {
		t.Skipf("SQL Server not reachable (%v) — start the QA bench's sqlserver profile", err)
	}
	...
	dbName := "omnicore_sqlserver_test_" + strings.ReplaceAll(uuid.NewString(), "-", "")
	if _, err := admin.ExecContext(ctx, "CREATE DATABASE "+dbName); err != nil {
		_ = admin.Close()
		t.Fatalf("CREATE DATABASE %s: %v", dbName, err)
	}
	testDSN = sqlserverAdminDSN() + ";database=" + dbName

	t.Cleanup(func() {
		c, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer cancel()
		_, _ = admin.ExecContext(c,
			"ALTER DATABASE "+dbName+" SET SINGLE_USER WITH ROLLBACK IMMEDIATE; DROP DATABASE IF EXISTS "+dbName)
		_ = admin.Close()
		testDSN = ""
	})
	return testDSN
...
// Postgres/MySQL parity for a cross-aggregate reference.
func TestSQLServerEngine_SecondaryUUIDColumn(t *testing.T) {
	eng, raw := setup(t)
	ctx := ctxFor()
...
func TestNew_AppliesPoolConfig(t *testing.T) {
	...
	eng, err := New(ctx, core.EngineConfig{DSN: newTestSQLServerDB(t), Pool: core.PoolConfig{MaxOpenConns: 7, MaxIdleConns: 3}})
	if err != nil {
...
func setup(t *testing.T) (*Engine, *sql.DB) {
	t.Helper()
	ctx := context.Background()
	testDB := newTestSQLServerDB(t)
	eng, err := New(ctx, core.EngineConfig{DSN: testDB})
	if err != nil {
		t.Fatalf("New on the test database: %v", err)
	}
	raw, err := sql.Open("sqlserver", testDB)
	if err != nil {
	...
	t.Cleanup(func() {
		// The throw-away database is dropped by newTestSQLServerDB's cleanup
...
func outboxCount(t *testing.T, raw *sql.DB, eventType, aggID string) int {
	...
	var n int
	if err := raw.QueryRow(
		`SELECT COUNT(*) FROM outbox WHERE event_type = @p1 AND aggregate_id = @p2`, eventType, aggID,
	).Scan(&n); err != nil {
...
func TestSQLServerEngine_WritePath(t *testing.T) {
	eng, raw := setup(t)
	ctx := ctxFor()
...
// TestSQLServerEngine_UpdateMatchSemantics covers the RowsAffected-driven match
// detection:
...
//     SQL Server rows-affected counts MATCHED rows natively (the row is
//     rewritten even when values are identical), so an idempotent PUT is not
//     mistaken for a missing row without any DSN flag (MySQL needs
//     clientFoundRows for the same guarantee).
func TestSQLServerEngine_UpdateMatchSemantics(t *testing.T) {
	eng, _ := setup(t)
	ctx := ctxFor()
...
// TestSQLServerEngine_FindByID proves the read seam: the framework's
// AggregateLoader + criteria engine run over the SQL Server engine via the
// neutral Querier/Dialect — the BINARY(16) id is encoded into the WHERE arg
// and decoded back on scan, so a UUID v7 round-trips through a real read.
func TestSQLServerEngine_FindByID(t *testing.T) {
	eng, _ := setup(t)
	ctx := ctxFor()
...
func setupAgg(t *testing.T) (*Engine, *sql.DB) {
	...
	raw, _ := sql.Open("sqlserver", testDB)
	for _, stmt := range []string{
		`CREATE TABLE accts (
...
// TestSQLServerEngine_AggregateRoundTrip writes an aggregate (root + 2
// children) via the engine and reads it back via the loader — proving the
// aggregate write path (child ParentID injection as BINARY(16), one TX) and the
// aggregate read path (batched child load) on SQL Server — then archives it
// (cascade) and deletes it (ParentID ON DELETE CASCADE).
func TestSQLServerEngine_AggregateRoundTrip(t *testing.T) {
	eng, raw := setupAgg(t)
	ctx := ctxFor()
...
// TestSQLServerEngine_UniqueViolation proves the dialect-aware mapErr: a SQL
// Server 2627/2601 on the uniq_email constraint is classified by
// sqlserverDialect.IsUniqueViolation and mapped to the bound typed
// notification (a *core.InfrastructureError), the same 409-shaped outcome the
// other engines produce — instead of leaking the raw driver error as a 500.
func TestSQLServerEngine_UniqueViolation(t *testing.T) {
	eng, _ := setup(t)
	ctx := ctxFor()
