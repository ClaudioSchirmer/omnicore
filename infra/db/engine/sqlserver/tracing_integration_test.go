...
// Opening the engine with tracing ON (the otelsql path) yields a working
...
//
//	go test -tags=integration,sqlserver ./infra/db/engine/sqlserver/ -run TracingOpen -count=1
func TestSQLServerEngine_TracingOpenWorks(t *testing.T) {
	// setup() provisions flat_persons + outbox and yields a (tracing-off)
	// engine it owns; here we open a SECOND engine with tracing ON over the
	// same DB.
	_, _ = setup(t)
