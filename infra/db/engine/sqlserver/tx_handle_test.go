...
// On an in-TX hook failure the SQL Server engine must emit the best-effort
...
func TestFireHook_LogsAndPropagatesError(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			slot: "afterBegin",
			fire: func(e *Engine, ctx persistence.RequestContext, hook core.WriteHook, hctx write.HookContext) error {
				return e.FireAfterBegin(ctx, sqlserverTx{}, nil, hook, hctx)
			},
		},
		{
			name: "beforeCommit",
			slot: "beforeCommit",
			fire: func(e *Engine, ctx persistence.RequestContext, hook core.WriteHook, hctx write.HookContext) error {
				return e.FireBeforeCommit(ctx, sqlserverTx{}, nil, domain.NewID("00000000-0000-0000-0000-000000000001"), hook, hctx)
			},
		},
	}
...
func TestFireHook_NilHookIsSilent(t *testing.T) {
	...
	if err := e.FireAfterBegin(ctx, sqlserverTx{}, nil, core.WriteHook{}, hctx); err != nil {
		t.Fatalf("nil AfterBegin should be a no-op, got %v", err)
	}
	if err := e.FireBeforeCommit(ctx, sqlserverTx{}, nil, domain.NewID("x"), core.WriteHook{}, hctx); err != nil {
