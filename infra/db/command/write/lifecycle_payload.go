...
// historical structural keys (the ID and, for a SharedBase role with a
// separate ParentID column, the ParentID to the shared identity) — a contract external
// consumers cascade on, so these keys only GROW (the payload adds the _ids
// block on top, see outbox_payload.go).

// sharedBaseFKField resolves the role's separate ParentID column and the
// deterministic base id for payload purposes. ok=false when the schema has no
// shared base, when the role shares the base's id as its own ID (the id
...
// — the ID under its physical column name and, for a SharedBase role with a
// separate ParentID column, the ParentID to the shared identity.
func deleteKeysPayload(schema *TableSchema, src domain.Entity, id string) domain.Fields {
	keys := domain.Fields{schema.IDColumn(): domain.NewID(id)}
