...
// (warm identity) keyed on the deterministic ID — NEVER a DB-native upsert.
//
...
// the natural-key ID) hijacked the write on MySQL: a new-identity POST whose
// email already existed matched the email unique key, ran the UPDATE branch on
// the WRONG persons row, never inserted the new base, and the role ParentID failed →
...
// The warm path (identity already exists) updates the shared fields by ID — an
